...
CountTable <- read.csv("normalizedcounts_combinedTEMTRM_60coregenes_Bld_CD8BMLNSpl.csv", header=T, row.names=1)

head(CountTable)
integernorm_counts <- round(CountTable)
Samples <- data.frame(row.names=colnames(integernorm_counts), condition=as.factor(c(rep("spl",3),rep("LN",3),rep("BM",3),rep("bld",3))))
...
benstyle.PCA = function(object, intgroup="condition", ntop=500, x=1, y=2, returnData=FALSE)
{
  ...
  kmeans_data = as.matrix(d[,c("PC1", "PC2")])
  
  centers = kmeans(kmeans_data, 2)
